...
subtitle = imfplotly.Heading2('Prepared by Bank of Mauritius')
object_desc.append(subtitle)
...
ax = sns.distplot(df_des['FX log returns'], hist=True, label='KDE',bins =100, ax=_ax)
plt.close()
...
fig = imfplotly.create_fig(forecasted_mean_w_df[xcl_df],
                           ...
                           xangle=-40,xnticks=len(forecasted_mean_w_df)+1,height= 900,
...
forecasted_density_w_df_plot.loc['Weighted Combination'] = pd.Series(forecasted_density_w_df_final_2.copy().sum())
...
imfplotly.create_html('html_report/master.html',
                      title='Mauritius Report Main Page',
                      links=links)
