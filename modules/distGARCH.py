...
class DistGARCH(object):
    ...
    def fit(self, cov_type='robust', disp='off', update_freq=1, first_obs=None, last_obs=None):
        return(DistGARCHFit(self, cov_type, disp, update_freq, first_obs, last_obs))
    ...
    def plot_description(self, start_date=None,
        ...
        # ax3 = sns.distplot(data[self.depvar], label='Kernel density estimate')
...
class DistGARCHFit(object): # Fitted class for the DistGARCH class
    ...
    # Initialization
    def __init__(self, DistGARCH, cov_type, disp, update_freq, first_obs, last_obs):
...
class DistGARCHForecast(object): # Forecast class for the DistGARCHFit class
    ...
    def __init__(self, DistGARCHFit, start_date, horizon, fmethod,
        ...
        if self.exog_l == None:
            ...
            for var in self.exog_l:
                # val_df = self.df_exog.loc[self.start_date:, [var]]
                # self.exog_fcast_d[var] = val_df.iloc[0:self.horizon,:].values
        ...
        q_l = [0.005, 0.01] + [round(x,3) for x in np.arange(0.025,1,0.025)] + [0.99]
    ...
    def VaR_FXI(self, qv_l=[0.025, 0.975]):
        ...
        if self.level: # If level, add it
            dv['Level Below'] = np.nan
            dv.loc[c_below, 'Level Below'] = dv.loc[c_below, self.level]

            dv['Level Above'] = np.nan
            dv.loc[c_above, 'Level Above'] = dv.loc[c_above, self.level]
    ...
    def plot_pdf_rule(self, fdate=None, q_low=0.05, q_high=0.95,
                      ...
                      sample_lim=0.1, ax=None, mode_val=False):
        """ 
        ...
        # Mode
        ax.text(x_mode, y_mode , 'Mode', color='darkred',
